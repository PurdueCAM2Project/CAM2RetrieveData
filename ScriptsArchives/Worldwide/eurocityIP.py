...
"""
-----------------------------------------------------------------------------
Descriptive Name: eurocityIP.py
Author          : (rewritten by Chan Weng Yan)					      
Contact Info    : cwengyan@purdue.edu
Date Written    : June 23,2016
Description     : Parse several european non traffic IP cameras
Command to run  : python eurocityIP.py > euroErrorIP
script
Output          : country#city#snapshot_url#latitude#longitude to 
		  <list_eurocityIP.txt>
		  urls that raise errors will be redirected to <euroErrorIP>
Note            : 1) this website uses a lot of non-ascii (unicode) characters.
		  ...
		  2) This script only gets all IP cameras, another script
		     is written to get NON_IP ones
...
def gotoCountry(baseurl,countrylist,countryname,f):
	...
	for country in countrylist:
		...
		individualImage(cityname,citylist,countryname[iter_country],f,urlIP)
		iter_country += 1
	urlIP.close()

def individualImage(cityname,citylist,country,f,urlIP):
