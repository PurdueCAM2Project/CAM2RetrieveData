...
class Camera(object):
    ...
    def connect(self):
        # This function is used to assess the time of connecting to camera.
        # This function is used when one does not want to download an image from camera.
        raise NotImplementedError("get_connection_time method must be overridden")
...
class IPCamera(Camera):
    ...
    def connect(self):
        ...
        # This function is used when one does not want to download an image from camera.
        try:
            conn = httplib.HTTPConnection(self.ip,self.port,timeout = 5) #port?
            conn.request("GET",self.image_path)
            resp = conn.getresponse()
        except Exception as e:
            return {'ID': self.id,'HTTP Error': e}
        else:
            return {'ID': self.id,'HTTP Response': resp.status}
...
class NonIPCamera(Camera):
    ...
    def connect(self):
        ...
        # This function is used when one does not want to download an image from camera.
        try:
            resp = urllib2.urlopen(self.url, timeout=5)
        except urllib2.HTTPError as e:
            return {'ID': self.id,'HTTP Error': e.code}
        except urllib2.URLError as e:
            return {'ID': self.id, 'URL Error': e}
        else:
            return {'ID':self.id,'URL Response':resp.code}